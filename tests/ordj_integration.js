...
// ===== FUZZ 1: alle 360 ord har en sætning og kan laves stavelser =====
const allW = Object.keys(WORDS);
check('360 ord i spillet', allW.length === 360, allW.length);
...
check('stavelses-deling rekonstruerer ordet (alle 360)', noSyl.length === 0, JSON.stringify(noSyl.slice(0,5)));
let duplicateSent = {};
let dupes = [];
...
// ===== FUZZ 2: alle 360 ord matcher mindst ét mønster (ellers kan de ikke læres adaptivt) =====
let unmatched = allW.filter(w => !SPELL_PATTERNS.some(p => p.test(w)));
check('alle 360 ord fanges af mindst ét mønster', unmatched.length === 0, JSON.stringify(unmatched));
...
// ===== FUZZ 4: alle 36 verdener er komplette =====
check('36 verdener', WORLDS.length === 36, WORLDS.length);
check('36 drager', BOSSES.length === 36, BOSSES.length);
let badWorld = [];
WORLDS.forEach((w, i) => {
...
check('alle 36 verdener har 10 gyldige ord + drage', badWorld.length === 0, JSON.stringify(badWorld.slice(0,6)));
check('360 ord i verdenerne til sammen', WORLDS.reduce((n, w) => n + w.words.length, 0) === 360, WORLDS.reduce((n, w) => n + w.words.length, 0));
let worldDupes = [];
...
// ===== INTEGRATION: alle 36 drager har gyldig styrke =====
let badBoss = [];
BOSSES.forEach((b, i) => { if (!b.name || !b.power || b.power < 5 || b.power > 200) badBoss.push(i + ':' + JSON.stringify(b).slice(0,40)); });
check('alle 36 drager har navn + rimelig styrke', badBoss.length === 0, JSON.stringify(badBoss.slice(0,4)));

// ===== INTEGRATION: verdens-valg + progression stemmer med 36 verdener =====
...
/* De 10 nye verdener (27-36) skal kunne vælges lige så frit som alle andre — hele
   Robins klasse skal kunne teste en hvilken som helst af de 36 verdener. */
check('verden 27 (Dobbelt-bjerget) kan vælges fra start', canEnterWorld(26) === true, canEnterWorld(26));
check('verden 36 (Drømme-tårnet) kan vælges fra start', canEnterWorld(35) === true, canEnterWorld(35));
check('verden 36 er ikke NÅET fra start', worldReached(35) === false, worldReached(35));
check('ugyldigt indeks (36) kan ikke vælges', canEnterWorld(36) === false, canEnterWorld(36));
check('ugyldigt indeks (99) kan ikke vælges', canEnterWorld(99) === false);
